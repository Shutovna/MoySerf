import {useEffect, useState} from "react";
import useStatsService from "../../services/StatsService.jsx";
import {Card, Row} from "react-bootstrap";
import useUserStatsService from "../../services/UserStatsService.jsx";

const MyReferalsCount = () => {
    const {getMyReferalsCount} = useUserStatsService();
    const [referalsCount, setReferalsCount] = useState(0);

    useEffect(() => {
        getMyReferalsCount()
            .then(res => setReferalsCount(res));

    }, [])

    return <>
        <Card className="custom-card">
            <Card.Body>
                <Row>
                    <div className="col-6 pe-0">
                        <p className="mb-2">
                            <span className="fs-16">Всего рефералов</span>
                        </p>
                        <p className="mb-2 fs-12">
                            <span className="fs-25 fw-semibold lh-1 vertical-bottom mb-0">{referalsCount}</span>
                        </p>
                    </div>
                    <div className="col-6">
                        <p className="main-card-icon mb-0">
                            <i className='bx bx-male-female fs-34'></i>
                        </p>
                    </div>
                </Row>
            </Card.Body>
        </Card>
    </>
}
export default MyReferalsCount;